...
$query = mysql_query("SELECT * FROM `services` WHERE `status` = '1' ORDER by `service_id` DESC");
...
function creat_order($user_id, $current_date, $link, $rate, $start_count, $quantity, $type, $status, $remains) {
	$user_id = (int)$user_id;
	$current_date = santizie($current_date);
	$current_date = santizie($current_date);
	$link = santizie($link);
	$rate = santizie($rate);
	$start_count = santizie($start_count);
	$quantity = santizie($quantity);
	$type = santizie($type);
	$status = santizie($status);
	$remains = santizie($remains);
...
function orders_history($user_id, $limit) {
$user_id = (int)$user_id;
$query = mysql_query("SELECT * FROM `orders` WHERE `user_id` = '$user_id' ORDER BY `order_id` DESC $limit") or die(mysql_error());
while ($row = mysql_fetch_assoc($query)) {
echo '
<tr>
...
function login_hash($login_hash) {
	if (isset($_SESSION['login_hash']) === true) {
		if ($login_hash != $_SESSION['login_hash']) {
			session_destroy();
			header("location: http://".$_SERVER['HTTP_HOST']."/login.php?error=2");
		}
	} else {
		session_destroy();
		header("location: http://".$_SERVER['HTTP_HOST']."/login.php?error=2");
...
function get_user_tickets($user_id, $limit) {
	$user_id = (int)$user_id;
	$query = mysql_query("SELECT * FROM `tickets` WHERE `user_id` = '$user_id' ORDER by `ticket_id` DESC $limit") or die(mysql_error());
	while ($row = mysql_fetch_assoc($query)) {
	echo '
	<tr>
	<td>'.$row['ticket_id'].'</td>
